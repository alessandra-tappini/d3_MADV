...
## This is the written guide for the second half - part one of L2. Please, refer to the slides for an in-depth theoretical explanation.
...
- We start by working in the `line-chart.js` file. First, we load the dataset with `d3.csv()`:
...
- Let's define the scale for the values on the xAxis. We are going to use the `scaleTime()` scale of D3. It takes a continous input and returns a continous output, but the data it manipulates are time-related data.
...
- For the yAxis scale, we also need a continuous to continuous scale. We use the `scaleLinear()`. To compute the domain, we do some preproccessing by computing the max value from the `max_temp_F` variable.
...
- Now, we want to display an abbreviation for the names of the months. We can change the format of the axis labels with the method `axis.tickFormat()`.
...
- Now the labels are correctly formatted but their poisition is incorrect. We want to move them in the middle of two ticks.
...
- To create our line chart, we first pass through an intermediate step.
- The strategy is the following: we first draw a scatter plot with our data points. Then, we interpolate these points with a line generator function from d3 modules, which simplifies the writing of an svg path.

# 2.1 Scatter Plot

- As a first step, we draw the scatter plot by using the data binding pattern.
...
- We append a path to the `innerChart` and we set its `d` attribute by calling the line generator and passing the dataset as an argument.
...
- We can obtain a more pleasing visualization if, instead of a simple line, we use a `curve generator`.
...
- Drawing an area in D3 is very similar to the process of drawing a line or a curve.
...
- In this section we will learn how to draw a pie chart (actually, a donut chart) in D3. We will use a dataset on `daily precipitations` in New York.
- Let's start by opening the `arc.js` file.
- We load this new dataset using the `d3.csv()` fetch function, as usual.
- Then, we will pass the dataset to a method that manages the visualization. We call this method `drawArc()`
...
- First, we setup the chart container:
  - We set the `width` and the `height` values for this new container.
  - We create a responsive container, as usual, by using `viewBox`
...
- We do this because we are going to use a polar coordinates system. In this way, the position of the arcs will be set, by default, to the center of the chart.
...
- Before appending our pie chart, we compute the angle for the days with precipitations.
- We filter the days in our year with a precipitations value greater than zero by using the `filter()` method of JS.
...
- Finally, we can generate the remaining part of the chart.
