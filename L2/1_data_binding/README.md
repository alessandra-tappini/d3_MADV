...
## This is the written guide for the first half of L2. Please, refer to the slides for an in-depth theoretical explanation.
...
- We need now to convert the values of `count` from strings to numbers. To convert it, we use the `+` operator. We can directly do this (and test it), in the callback function of `d3.csv()`:
...
- Probably, we are also interested in the `max` and/or `min` value in our dataset. To retrieve them, we can use one of these `d3` methods.
...
- In the end, we add a method which takes the data and creates a visualization. We can call this method `createViz()`.
...
- Now we can interact with the data in the svg. First, we assign a class to every single rect:
...
- In order to add the labels to our chart, we need to refactor our code a bit. We create a group for every data point with this scheme in mind: **d.technology - bar - d.count**
...
- Now we can add the labels. We start with the technology label, on the left side:
...
- To add the count labels, on the right part of the code, we need to add:
...
- Finally, we add a vertical black line to separate these labels from the starting point of the bar:
